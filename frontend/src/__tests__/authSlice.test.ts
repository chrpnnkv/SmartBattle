...
// Локальная in-memory имитация localStorage. Подкладываем её в globalThis ДО
// импорта authSlice — иначе в test-окружении (где globalThis.localStorage
// может отсутствовать) initialState ломается на первом же localStorage.getItem.
...
// Импортируем slice ПОСЛЕ stub — это критично, потому что initialState читает
// localStorage.getItem на этапе вычисления модуля.
...
describe('authSlice.changePassword', () => {
  ...
  it('keeps state intact if fulfilled payload is undefined', () => {
    // Защита от undefined payload — reducer не должен бросать TypeError.
    const fulfilledAction = {
      type: changePassword.fulfilled.type,
      payload: undefined,
    };
